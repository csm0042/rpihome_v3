...
class TestASIntToDatabase(unittest.TestCase):
    ...
    def test_process_db_lsu(self):
        """ test """
        self.log.debug('Running "test_process_db_lsu" ************************')
    ...
    def test_process_db_rc(self):
        """ test """
        self.log.debug('Running "test_process_db_rc" ************************')
        self.dummy_msg_in = '101,127.2.2.1,15000,192.168.86.12,16000,' \
                            '100,device01'
        self.dummy_msg_out_list = process_db_rc(
            self.log,
            self.dummy_msg_in,
            self.service_addr
        )
        self.rc_message.complete = copy.copy(self.dummy_msg_out_list[0])
        self.assertEqual(len(self.dummy_msg_out_list), 1)
        self.assertEqual(self.rc_message.ref, '101')
        self.assertEqual(self.rc_message.dest_addr, '127.0.0.1')
        self.assertEqual(self.rc_message.dest_port, '27011')
        self.assertEqual(self.rc_message.source_addr, '192.168.86.12')
        self.assertEqual(self.rc_message.source_port, '16000')
        self.assertEqual(self.rc_message.msg_type, '100')
        self.assertEqual(self.rc_message.dev_name, 'device01')


    def test_process_rc_ack(self):
        """ test """
        self.log.debug('Running "test_process_db_rc_ack" ********************')
        self.dummy_msg_in = '101,127.2.2.1,15000,192.168.86.12,16000,' \
                            '103,42,device01,on,2017-01-02 03:30:00,'
        self.dummy_msg_out_list = process_db_rc_ack(
            self.log,
            self.ref_num,
            self.devices,
            self.dummy_msg_in,
        ...
        #self.assertEqual(len(self.dummy_msg_out_list), 2)
        #self.uc_message.complete = copy.copy(self.dummy_msg_out_list[0])
        #self.sds_message.complete = copy.copy(self.dummy_msg_out_list[1])


    def test_process_uc(self):
        """ test """
        self.log.debug('Running "test_process_db_uc" ************************')
        self.dummy_msg_in = '101,127.2.2.1,15000,192.168.86.12,16000,' \
                            '104,42,2017-01-02 03:04:05'
        self.dummy_msg_out_list = process_db_uc(
            self.log,
            self.dummy_msg_in,
            self.service_addr
        )
        self.uc_message.complete = copy.copy(self.dummy_msg_out_list[0])
        self.assertEqual(len(self.dummy_msg_out_list), 1)
        self.assertEqual(self.uc_message.ref, '101')
        self.assertEqual(self.uc_message.dest_addr, '127.0.0.1')
        self.assertEqual(self.uc_message.dest_port, '27011')
        self.assertEqual(self.uc_message.source_addr, '192.168.86.12')
        self.assertEqual(self.uc_message.source_port, '16000')
        self.assertEqual(self.uc_message.msg_type, '104')
        self.assertEqual(self.uc_message.dev_id, '42')
        self.assertEqual(self.uc_message.dev_processed, '2017-01-02 03:04:05')
